...
class STPageSCroll: UIScrollView {


    fileprivate var titles: [String]!
    fileprivate var views:[UIView]!
    fileprivate var topView: UIView!
    fileprivate lazy var optionView: STOptionView = {
        let newOptionView: STOptionView = STOptionView(frame: CGRect(x: 0, y: 0, width: ScreenW, height: 40), type: .line)
        ...
        line.frame = CGRect(x: 0, y: 39.5, width: ScreenW, height: 0.5)
        newOptionView.addSubview(line)

        return newOptionView
    }()
    fileprivate lazy var pageView: STPageView = {
        let aPageView = STPageView()
        aPageView.frame = CGRect(x: 0, y: 40, width: ScreenW, height: ScreenH - 40)
    ...
    private func setupSubView() {
        ...
        let pageView = UIView(frame: CGRect(x: 0, y: midView.frame.maxY, width: ScreenW, height: ScreenH - 64 - 40))
        addSubview(pageView)
        ...
        contentSize = CGSize(width: ScreenW, height: topView.frame.height + 40)
    ...
    private func mideView() -> UIView {
        let midView = UIView(frame: CGRect(x: 0, y: (topView.frame.maxY), width: ScreenW, height: 40))
    ...
    private func sateChange() {
        ...
        case .scrollToDown:
            UIView.animate(withDuration: 0.5, delay: 0, options: .curveEaseInOut, animations: {
                ...
                self.contentSize = CGSize(width: ScreenW, height: self.topView.frame.height + 40)
        ...
        case .scrollToUp:
            ...
            UIView.animate(withDuration: 0.35, delay: 0, options: .curveEaseInOut, animations: {
                ...
                self.contentSize = CGSize(width: ScreenW, height: self.topView.frame.height + 40 + ScreenH)
...
extension STPageSCroll: UIScrollViewDelegate {
...
//MARK: STOptionViewDelegate
extension STPageSCroll: STOptionViewDelegate {
    func numberOfItemsInOptionView(_ optionView: STOptionView) -> Int {
        return titles.count
    }
    func optionView(_ optionView: STOptionView, itemSizeOfIndex index: Int) -> CGSize {
    ...
    func optionView(_ optionView: STOptionView, cellPointConfig cellConfig: STOptionViewCellPoint) {
        let cell = cellConfig.pointee
        cell.textLabel.text = titles[cell.index]
    }
    func optionView(_ optionView: STOptionView, didSelectedItemOfIndex index: Int) {
        pageView.currentIndex = index
    }
}

//MARK: STPageViewDelegate
extension STPageSCroll: STPageViewDelegate {
    ...
    func pageView(_ pageView: STPageView, cellForPageViewAtIndex index: Int) -> STPageViewCell {
        let cell: STPageViewCell = pageView.dequeueReusableCell(withReuseIdentifier: "STPageViewCell", for: IndexPath(row: index, section: 0)) as! STPageViewCell
        cell.index = index
        let view = views[index]
        if view is STTableView {
            (view as! STTableView).scrolldidChangeClosures(closures: { (scrollView, isStop) in
                let offsetY = scrollView.contentOffset.y
                print(offsetY)
                if offsetY <= 0 {
                    self.optionView.frame = CGRect(x: 0, y: -offsetY, width: ScreenW, height: 40)
                    if offsetY <= -80 {
                        if !scrollView.isDragging {
                ...
                if isStop {
                    UIView.animate(withDuration: 0.2, animations: {
                        self.optionView.frame = CGRect(x: 0, y: 0, width: ScreenW, height: 40)
                    })
                }

            })
        } else {
            (view as! STWebView).scrolldidChangeClosures(closures: { (scrollView, isStop) in
                let offsetY = scrollView.contentOffset.y
                print(offsetY)
                if offsetY <= 0 {
                    self.optionView.frame = CGRect(x: 0, y: -offsetY, width: ScreenW, height: 40)
        ...
        view.frame = CGRect(x: 0, y: 0, width: ScreenW, height: ScreenH - 40 - 64)
