...
public typealias STOptionViewCellPoint = AutoreleasingUnsafeMutablePointer<STOptionViewCell>
@objc public protocol STOptionViewDelegate: NSObjectProtocol {
    ...
    @objc optional func optionView(_ optionView: STOptionView, didSelectedItemOfIndex index: Int)
}
...
public class STOptionView: UICollectionView {
    public weak var optionViewDelegate:STOptionViewDelegate? {
        didSet{
            setOffset()
            setLineView()
        }
    ...
    fileprivate lazy var longLineView: UIView! = {
        ...
        longLine.frame = CGRect(x: 0, y: self.frame.maxY - self.frame.origin.y - 1, width: ScreenW, height: 1)
        return longLine
    }()
    ...
    init(frame: CGRect) {
        ...
        delegate = self
        bounces = true
        register(STOptionViewCell.self, forCellWithReuseIdentifier: "STOptionViewCell")
...
extension STOptionView {
    ...
    private func handleColorOfDecimal(_ decimal: Float) -> UIColor {
        ...
        return STColor(startColor: startColor!, endColor: endColor!, decimal: CGFloat(decimal))
    }
    ...
    private func cellForOptionViewAtIndex(_ index: Int) ->STOptionViewCell {
        let cell = cellForItem(at: IndexPath(row: index, section: 0)) as! STOptionViewCell
        return cell
    }
}

//MARK: UICollectionViewDataSource and UICollectionViewDelegate and UICollectionViewDelegateFlowLayout
extension STOptionView: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        var cell: STOptionViewCell = dequeueReusableCell(withReuseIdentifier: "STOptionViewCell", for: indexPath) as! STOptionViewCell
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if let preCell: STOptionViewCell =  cellForItem(at: IndexPath(row: selectedIndex, section: 0)) as! STOptionViewCell?  {
            if type == .line {
        ...
        if let currentCell: STOptionViewCell = cellForItem(at: indexPath) as! STOptionViewCell? {
            if type == .line {
        ...
        guard optionViewDelegate!.responds(to: #selector(STOptionViewDelegate.optionView(_:didSelectedItemOfIndex:))) else {
            return
        }
...
public class STOptionViewCell: UICollectionViewCell {
    var index: Int = 0
