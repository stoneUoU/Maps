...
class PopMenu: UIView, BlurViewProtocol{
    ...
    func showMenuAtView(containerView:UIView){
        ...
        self.blurView.showBlurViewAtView(currentView: self);
    }
    ...
    func showItems(){
        ...
        for menuItem in self.items {
            ...
            case .Diffuse:
                fromRect.origin.x = self.bounds.midX - CGFloat(itemWidth/2.0);
                fromRect.origin.y = self.startPoint.y;
            }
    ...
    func hideItems(){
        for index in 0..<self.items.count{
            ...
            case .Diffuse:
                toRect.origin.x = self.bounds.midX - CGFloat(menuButton.bounds.size.width/2.0);
                toRect.origin.y = self.endPoint.y;
            }
    ...
    func initAnimation(toRect toRect:CGRect, fromRect:CGRect, atView:UIView, beginTime:Double){
        ...
        UIView.animate(withDuration: 0.3, delay: beginTime, options: .curveEaseInOut, animations: {
    ...
    func getFrameOfItem(AtIndex index:Int, itemWidth:Float) -> CGRect {
        ...
        let insetY:Float = Float(UIScreen.main.bounds.size.height) * 0.1 + 64;
...
class MenuButton: UIView{
    ...
    func initContent(){
        ...
        self.iconImageView.center = CGPoint(x: self.bounds.midX, y: self.iconImageView.bounds.midY);
        self.addSubview(self.iconImageView);
        
        self.titleLabel = UILabel(frame: CGRect(x: 0, y: self.iconImageView.frame.maxY, width: self.bounds.width, height: 35));
