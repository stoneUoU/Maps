...
final class ViewPlaceholder: UIView {
	...
	private func configureTitles() {
		...
		noConnectionTitleLabel?.text 		= "网络无连接"
		noConnectionSubtitleLabel?.text 	= "请稍后再试"
		noConnectionRetryButton?.setTitle("重新加载", for: .normal)
		noDataTitleLabel?.text 				= "暂无数据"
		noDataSubtitleLabel?.text			= "请稍后再试"
	}
	
	public func configure(state: PlaceholderState) {
		switch state {
		case .noConnection:
