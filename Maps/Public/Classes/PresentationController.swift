...
class PresentationController: UIPresentationController {
    ...
    private lazy var dismissButton: UIButton = {
        let x = (Constants.ScreenW + ADConfig.shared.ADViewSize.width) / 2
        let y = (Constants.ScreenH - ADConfig.shared.ADViewSize.height) / 2
        let dismissButton = UIButton(frame: CGRect(x: x - 44, y: y, width: 44, height: 44))
        if ADConfig.shared.closeButtonPosition == .bottom {
            dismissButton.frame.origin = CGPoint(x: (Constants.ScreenW - 44) / 2, y: (Constants.ScreenH + ADConfig.shared.ADViewSize.height) / 2)
        }
