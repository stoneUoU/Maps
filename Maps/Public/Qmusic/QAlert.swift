...
public typealias AcceptBlock = ()->Void  //接受Block
public typealias DenyBlock = () ->Void   //拒绝Block
public typealias ForceBlock = () ->Void   //强制Block

public class TipAlert: UIView {

    var btnTitleArr:NSArray = []/**< 按钮标题数组，传入两个即可 */
    var topImage:UIImage?    /**< 顶部图片 */
    let contextStr:String    /**< 内容 */
    let alignment:NSTextAlignment
    public var acceptBlock:AcceptBlock? = nil
    public var denyBlock:DenyBlock? = nil
    public var forceBlock:ForceBlock? = nil
    ...
    /**
     创建按钮
     */
    fileprivate func addAlertButtons(btnTitleArr:NSArray){
        ...
        for index in 0..<btnTitleArr.count{
            ...
            if btnTitleArr.count == 2{
                ...
                button.titleLabel?.font = UIFont.systemFont(ofSize: 15)
                button.addTarget(self, action: #selector(TipAlert.buttonClick(_:)), for: UIControlEvents.touchUpInside)
                button.frame = CGRect(x: ((alertSize?.width)! - 120)/2, y: bgImageView!.frame.height-50, width: 120, height: 38)
                button.backgroundColor = getColor("39C27E")
                button.setTitleColor(UIColor.white, for: UIControlState())
                bgImageView?.addSubview(button)
            }
    ...
    public func show(closeBtnHide:Bool,btnTitleArr:NSArray){
        let alertWindow = UIApplication.shared.keyWindow
        self.addAlertImages()
        self.addAlertLabels()
        self.addAlertButtons(btnTitleArr:btnTitleArr)
        closeBtnHide == false ? self.addCloseBtn() : STLog("Hide Btn")
