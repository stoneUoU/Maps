...
//    var centerT = UILabel()
    ...
    override func draw(_ rect: CGRect) {
        var scanZoneSize = CGSize(width: ScreenInfo.width - self.defaultBothSideWidth * 2, height: ScreenInfo.width - self.defaultBothSideWidth * 2)
        if scanType == .barCode {
            scanZoneSize = CGSize(width: ScreenInfo.width - self.defaultBothSideWidth * 2, height: 150)
        }
        //获取扫描区域的坐标
        let x = self.defaultBothSideWidth
        let y = ScreenInfo.height / 2 - scanZoneSize.height / 2
...
//    }
    ...
    func addRedLineWithBarCodeScan() {
        if self.redLine == nil {
            ...
            self.redLine?.bounds = CGRect(x: 0, y: 0, width: ScreenInfo.width - 60 * 2 - 20, height: 1.5)
    ...
    func creatDrawLine() {
        ...
        let rect = CGRect(x: 60 + 10, y: (ScreenInfo.height - (ScreenInfo.width - 60 * 2)) / 2, width: ScreenInfo.width - 60 * 2 - 20, height: 2)
        ...
        transitionAnimation.fromValue = NSValue.init(cgPoint: CGPoint(x: 60 + (ScreenInfo.width - 60 * 2) / 2, y: (ScreenInfo.height - (ScreenInfo.width - 60 * 2)) / 2))
        transitionAnimation.toValue = NSValue.init(cgPoint: CGPoint( x: 60 + (ScreenInfo.width - 60 * 2) / 2, y: ScreenInfo.height / 2 + (ScreenInfo.width - 60 * 2) / 2))
    ...
    func creatOtherView(_ scanRect : CGRect) {
        ...
        topView = UIView.init(frame: CGRect(x: 0, y: 0, width: ScreenInfo.width, height: scanRect.origin.y))
        ...
        bottomView = UIView.init(frame: CGRect(x: 0,y: scanRect.maxY, width: ScreenInfo.width, height: ScreenInfo.height - scanRect.maxY - 100))
        bottomView!.backgroundColor = UIColor.black
        bottomView!.alpha = allAlpha
        self.addSubview(bottomView!)

        let detailLabel = UILabel.init(frame: CGRect(x: 0, y: 10, width: ScreenInfo.width, height: 20))
        ...
        //我的二维码
        let mineBtn = UIButton.init(frame: CGRect(x: (ScreenInfo.width - 150) / 2, y: 40, width: 150, height: 40))
    ...
    func setBottomBtnView() {
        ...
        btnBottomView = UIView.init(frame: CGRect(x: 0, y: ScreenInfo.height - btnBottomViewHeight, width: ScreenInfo.width, height: btnBottomViewHeight))
